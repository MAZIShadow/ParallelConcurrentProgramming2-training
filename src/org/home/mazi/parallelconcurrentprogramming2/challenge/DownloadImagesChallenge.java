...
class SequentialImageDownloader {
	...
	private int downloadImage(int imageNumber) {
		try {
			...
			try (BufferedInputStream in = new BufferedInputStream(photoURL.openStream())) {
				int bytesRead;
				totalBytes = 0;
...
class ParallelImageDownloader {
	...
	class DownloadImageTask implements Callable<Integer> {
	...
	public int downloadAll() {
		...
		int totalBytes = 0;
		for (int num : imageNumbers) {
			callables.add(new DownloadImageTask(num));
		}
		try {
